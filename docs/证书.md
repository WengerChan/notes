...
# 3、吊销证书
openssl ca -revoke example.crt
# 4、【可选】生成吊销证书的编号(如果是第一次吊销)
echo 00 > /etc/pki/CA/crlnumber
# 5、【可选】更新证书吊销列表
cd /etc/pki/CA/crl/
openssl ca -gencrl -out thisca.crl
# 6、【可选】如果需要，查看crl文件的内容：
...
* 示例1
    ...
    ```sh
    cp server.cert /etc/pki/ca-trust/source/anchors/   # -----BEGIN CERTIFICATE-----.
    cp server.cert /etc/pki/ca-trust/source/           # BEGIN TRUSTED
